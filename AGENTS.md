...
| Backend | Flag | Notes |
|---|---|---|
| Groq API | `--transcriber groq` | Default. Requires `GROQ_API_KEY`. Converts audio to 32 kbps MP3 and splits files over 20 MB into 15-minute chunks; each chunk's segments are saved to `chunk_NNN.json` so a failed request only re-uploads unfinished chunks. |
